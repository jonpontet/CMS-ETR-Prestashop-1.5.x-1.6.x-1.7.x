...
class ETransactionsKwixo
{
    ...
    public function __construct(ETransactionsConfig $config)
    {
        $this->_config = $config;
    }
    ...
    public function buildKwixoParams(Cart $cart, array $additionalParams = array())
    {
        ...
        $deliveryDelay = intval(Configuration::get('ETRANS_CARRIER_DAYS_'.$carrier->id));
        ...
        // Delivery Speed
        $deliverySpeed = Configuration::get('ETRANS_CARRIER_TYPE_'.$carrier->id);
        if (!in_array($deliverySpeed, array(1, 2))) {
            $deliverySpeed = 1;
        }

        // Delivery type
        $deliveryType = Configuration::get('ETRANS_CARRIER_TYPE_'.$carrier->id);
        ...
        $default = Configuration::get('ETRANS_DEFAULTCATEGORYID');
        if (empty($default)) {
            $default = 1;
        }
        foreach ($products as $product) {
            ...
            foreach ($categories as $category) {
                $value = Configuration::get('ETRANS_CAT_TYPE_'.$category['id_category']);
                if ($value) {
                    $selected = Configuration::get('ETRANS_CAT_TYPE_'.$category['id_category']);
                    break;
                }
            }
