...
class ETransactionsHelper extends ETransactionsAbstract
{
    ...
    private function _callDirect(Order $order, $type, $trxId, $callId, $amount, $cardType)
    {
        ...
        $message = 'Order %d: call E-Transactions - DEVISE=%s MONTANT=%s NUMAPPEL=%s NUMTRANS=%s TYPE=%s';
        ...
        $this->logDebug(sprintf('Order %d: E-Transactions error code %d', $order->id, $result['code']));
        $this->logDebug(sprintf('Order %d: E-Transactions data %s', $order->id, $result['data']));
    ...
    public function addOrderPayment(Order $order, $type, $params, $method)
    {
        ...
        if (Validate::isLoadedObject($cart)) {
            ...
            if (!$db->insert('etransactions_order', $data)) {
    ...
    public function addOrderRecurringDetails(Order $order, $amountPaid)
    {
        ...
        $db = new ETransactionsDb();
        if (!$db->insert('etransactions_recurring', $data)) {
            $this->logError(sprintf('Unable to save order payment for order %d: %s', $order->id, $db->getMsgError()));
        }
    }
    ...
    public function buildSystemParams(Cart $cart, array $card, $type, array $additionalParams = array())
    {
        ...
        if ($card['type_payment'] == 'KWIXO') {
            $kwixo = new ETransactionsKwixo($this->getConfig());
    ...
    public function isRecurring($orderId)
    {
        ...
        if (($details['payment_by'] == 'E-TransactionsRecurring')) {
            return true;
        }
    ...
    public function checkUrls(array $urls)
    {
        ...
        $client->setTimeout(5);
        $client->setUserAgent('E-Transactions');
    ...
    public function deleteRecurringPayment(Order $order, array $details)
    {
        ...
        // Update details
        $sql = 'UPDATE `%setransactions_order` SET `payment_status` = "canceled"'
            .' WHERE `id_order` = %d';
    ...
    {
        $sql = 'SELECT * FROM `%setransactions_card`';
    ...
    public function getOrderDetails($orderId)
    {
        ...
        $sql = 'SELECT * FROM `%setransactions_order` WHERE `id_order` = %d';
        $sql = sprintf($sql, _DB_PREFIX_, $orderId);
    ...
    public function getOrderPaymentBy($orderId)
    {
        ...
        $sql = 'SELECT `payment_by` FROM `%setransactions_order` WHERE `id_order` = %d';
        $sql = sprintf($sql, _DB_PREFIX_, $orderId);
        return Db::getInstance()->getValue($sql);
    }
    ...
    public function getOrderRecurringDetails($orderId)
    {
        ...
        $sql = 'SELECT * FROM `%setransactions_recurring` WHERE `id_order` = %d';
        $sql = sprintf($sql, _DB_PREFIX_, $orderId);
        return Db::getInstance()->getRow($sql);
    }
    ...
    public function getOrderTransactionId($orderId)
    {
        ...
        $sql = 'SELECT `id_transaction` FROM `%setransactions_order` WHERE `id_order` = %d';
        $sql = sprintf($sql, _DB_PREFIX_, $orderId);
        return Db::getInstance()->getValue($sql);
    }
    ...
    {
        $db = Db::getInstance();
        $sql = 'SELECT * FROM `%setransactions_card` WHERE `type_card` = "%s"';
    ...
    {
        $db = Db::getInstance();
        $sql = 'SELECT * FROM `%setransactions_card` WHERE `id_card` = %d';
    ...
    {
        return (Configuration::get('ETRANS_WEB_CASH_DIRECT') == 1 || Configuration::get('ETRANS_WEB_CASH_DIRECT') == 2);
    }
    ...
    public function makeCaptureAll(Order $order, array $details, $changestate = true)
    {
        ...
        // Success
        $this->logError(sprintf('Order %d: Success of E-Transactions call', $order->id));
        $message = $this->l('Capture operation:').chr(10).chr(13);
...
/*
        // Update database
        $sql = 'UPDATE `%setransactions_order` SET `payment_status` = "%s",'
...
*/
    ...
    public function makeRefundAmount(Order $order, array $details, $amount)
    {
        ...
        // Update database
        $sql = 'UPDATE `%setransactions_order` SET `refund_amount` = refund_amount + %d'
            .' WHERE `id_order` = %d';
    ...
    {
        $db = Db::getInstance();
        $sql = 'UPDATE `%setransactions_recurring` SET `number_term` = %d, `amount_paid` = %d WHERE `id_order` = %d';
    ...
    {
        $db = Db::getInstance();
        $sql = 'UPDATE `%setransactions_order` SET `initial_amount` = %d, `amount` = %d WHERE `id_order` = %d';
...
*/
    ...
    public function updatePayments($order, $details, $newAmount, $operationAmount, $amountScale, $currency)
    {
        ...
        // Update Payment
        $sql = 'UPDATE `%setransactions_order` SET `amount` = %d'
        . ' WHERE `id_order` = %d';
                    ...
                    foreach ($orderPayments as $orderPayment) {
                        if ($orderPayment->transaction_id == $details['id_transaction']) {
                            ...
                            if (!$order->addOrderPayment($operationAmount, $orderPayment->payment_method, $transactionId, $currency, null, $orderInvoice)) {
                                ...
                                if (!Db::getInstance()->execute($sql)) {
                                    $errors[] = 'The initial payment amount in E-Transactions could not be reset';
                                }
                                return array(
                                    'status' => 0,
                                    'error' => $errors,
    ...
    public function hasCartLocker($cartId, $transactionId)
    {
        ...
        $sql = 'SELECT COUNT(*) FROM `%setransactions_cart_locker` WHERE `id_cart` = %d AND `id_transaction` = "%s"';
    ...
    public function createCartLocker($cartId, $transactionId)
    {
        ...
        $db = new ETransactionsDb();
        if (!$db->insert('etransactions_cart_locker', $data)) {
